...
export default function VitrinaInventoryPage() {
  return (
    <PermissionGuard permission="canViewInventory">
      <VitrinaInventoryContent />
    </PermissionGuard>
  )
}

function VitrinaInventoryContent() {
  ...
  const loadData = async () => {
    try {
      ...
      if (categoriesRes.ok) {
        ...
        const vitrinaCategories = categoriesData
          .filter((cat: any) => cat.includes('VITRINA') || cat.includes('SERVICIOS'))
          .map((cat: any) => cat)
  ...
  const moveToCakeBar = async (productId: number) => {
    if (!permissions.canEditProducts) {
      alert('No tienes permisos para mover productos')
      return
    }

    if (!confirm('¿Mover este producto al inventario de Cake Bar?')) return
    ...
    try {
      ...
      if (response.ok) {
        setProducts(products.filter(p => p.id !== productId))
        alert('Producto movido al inventario de Cake Bar exitosamente')
      }
    } catch (error) {
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Inventario Vitrina</h1>
            <p className="text-gray-600">Gestión de productos y servicios de vitrina</p>
          </div>
        </div>
...
// Componente ProductRow (similar al anterior pero con checkbox y botón de mover)
function ProductRow({ 
  product, 
  categories, 
  isSelected,
  ...
  onMoveToCakeBar: (id: number) => void;
}) {
  const permissions = usePermissions();
  ...
  return (
    <tr className={`hover:bg-gray-50 ${!product.isActive ? 'bg-gray-50 opacity-75' : ''} ${isSelected ? 'bg-blue-50' : ''}`}>
        ...
        {product.isEditing && canEdit ? (
          <div className="space-y-2">
            <input
              ...
              step="0.01"
              value={product.price}
              onChange={(e) => onUpdateField(product.id, 'price', parseFloat(e.target.value))}
              className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
            />
            <input
              type="number"
              ...
              onChange={(e) => onUpdateField(product.id, 'specialPrice', e.target.value ? parseFloat(e.target.value) : null)}
        ...
        {product.isService ? (
          ...
          <div className="space-y-2">
            <input
              type="number"
              value={product.stock}
              onChange={(e) => onUpdateField(product.id, 'stock', parseInt(e.target.value))}
              className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
            />
            <input
              type="number"
              value={product.minStock}
              onChange={(e) => onUpdateField(product.id, 'minStock', parseInt(e.target.value))}
        ...
        <div className="flex items-center space-x-2">
          {product.isEditing ? (
            ...
            <>
              ...
              {canEdit && (
                <button
                  onClick={() => onMoveToCakeBar(product.id)}
                  className="p-1 text-purple-600 hover:bg-purple-50 rounded"
                  title="Mover a Cake Bar"
                >
